...
class AiPlayer(Player):
    ...
    def auto_shot_poker(self):
        ...
        if total_cards_cnt <= AiPlayer.MCT_THRESH:
            ...
            #手牌  , 其他人未知手牌 , 下家手牌数(不确定),  上一个人出的牌. 是否地主(不确定),
            print(chandcards, cunseen_cards, next_handcards_cnt, last_cg, (self.table.whose_turn - self.table.lord_turn + 3) % 3, (self.table.controller - self.table.lord_turn + 3) % 3, 100, 50, 500)
            caction = mcsearch(chandcards, cunseen_cards, next_handcards_cnt, last_cg, (self.table.whose_turn - self.table.lord_turn + 3) % 3, (self.table.controller - self.table.lord_turn + 3) % 3, 100, 50, 500)
